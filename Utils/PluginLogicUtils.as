...
int MapIndicesToId(int seriesIndex, int mapIndex, CheckTypes checkType)
{
    int mapBase = BASE_ID
        + seriesIndex * (MAX_MAP_LOCATIONS * MAX_MAPS_IN_SERIES)
        + mapIndex * (MAX_MAP_LOCATIONS);
...
vec3 MapIdToIndices(int id)
{
    ...
    int seriesIndex = num / (MAX_MAP_LOCATIONS * MAX_MAPS_IN_SERIES);
    int mapIndex    = (num % (MAX_MAP_LOCATIONS * MAX_MAPS_IN_SERIES)) / MAX_MAP_LOCATIONS; 
    int check       = num % MAX_MAP_LOCATIONS;

    return vec3(seriesIndex, mapIndex, check);
}
