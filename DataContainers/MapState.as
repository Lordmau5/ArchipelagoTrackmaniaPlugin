class MapState
{
    ...
    private int CalculateTargetTime()
    {
        ...
        int medalIndex          = int(Math::Floor(targetTimeSetting));
        int nextMedalIndex      = medalIndex + 1;
        float factor            = targetTimeSetting - Math::Floor(targetTimeSetting); // no frac function imma cry ;-;
        
        array<int> medalTimes   = {
            mapInfo.BronzeTime,
            mapInfo.SilverTime,
            mapInfo.GoldTime,
            mapInfo.AuthorTime,
        ...
        float interpolatedTime = Math::Lerp(medalTimes[medalIndex], medalTimes[nextMedalIndex], factor);
    ...
    void RequestThumbnail(bool delay = false)
    {
        ...
        if (delay)
        {
             //this is just a hacky way to not ddos tmx without adding a ton to this project i rly just wanna be doneaaaaaaaaa
            request.delayMS = 4000 * seriesIndex;
        } //this is just a hacky way to not ddos tmx without adding a ton to this project i rly just wanna be doneaaaaaaaaa
