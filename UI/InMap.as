void RenderMapUI()
{
    ...
    if (UI::Begin("Archipelago - HUD", isOpen, flags))
    {
        ...
        else
        {
            ...
            if (data !is null)
            {
                uint nextSeriesIndex = data.LatestUnlockedSeriesIndex() + 1;
                if (nextSeriesIndex < data.world.Length)
                {
                    UI::Text("Series " + (nextSeriesIndex + 1) + " Unlock Progress:");
                    UI::Indent();
                    RenderMedalProgress(GetProgressionTex(),50,data.items.GetProgressionMedalCount(), data.world[nextSeriesIndex].medalRequirement);
                    UI::Unindent();
                }
