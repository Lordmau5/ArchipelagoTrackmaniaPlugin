...
void Celebrate()
{
    // Remap volume to be from 0 -> 1
#if TMNEXT
    float gameVolume = (GetApp().SystemOverlay.MasterSoundVolume + 100) / 100.0;
#elif MP4
    ...
    // TODO: Set this to actual game resolution? Or does it scale?
    ...
    {
        bool leftSide = i < uint(confetti.Length / 2);

        float angle = Math::Rand(minAngle, maxAngle);
        float speed = Math::Rand(minSpeed, maxSpeed);

        vec2 position = vec2(leftSide ? 0 : refRes.x, refRes.y);
        vec2 velocity = vec2(leftSide ? Math::Cos(angle) : -Math::Cos(angle), -Math::Sin(angle)) * speed;
        ...
        @confetti[i] = ConfettiStrip(
            ...
            UI::HSV(
                Math::Rand(0.0, 1.0),
                Math::Rand(minSaturation, maxSaturation),
...
void CelebrationRender()
{
    ...
    for (uint i = 0; i < confetti.Length; i++)
    {
        if (confetti[i] is null) continue;
...
void CelebrationUpdate(float deltaTime)
{
    ...
    for(uint i = 0; i < confetti.Length; i++)
    {
        ...
        confetti[i].Update(deltaTime/1000.0);
    }
}