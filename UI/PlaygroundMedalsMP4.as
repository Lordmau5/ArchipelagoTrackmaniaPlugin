...
void DrawPlaygroundUI()
{
    ...
    {
        Log::Error("No viewport or overlays");
        return;
    }
    ...
    try
    {
        ...
        MedalRE.Hide();
        if (RaceEnd.IsVisible
            && cast<CControlContainer>(cast<CControlContainer>(RaceEnd.Childs[0]).Childs[1]).IsVisible
        )
        {
            DrawMedalSelection(MedalRE);
        }
        ...
        MedalGS.Hide();
        if (GhostSelection.IsVisible)
        {
            DrawMedalSelection(MedalGS);
        }
        ...
        if(cast<CControlContainer>(cast<CControlContainer>(BigMedals.Childs[0]).Childs[2]).IsVisible)
        {
            DrawBigMedals(BigMedalsParent);
        }
    }
    catch
    {
...
void DrawMedalSelection(CControlQuad@ Medal)
{
    ...
    nvg::Texture@ texture = GetNthBowTieTex(textureIndex);
    ...
    nvg::FillPaint(nvg::TexturePattern(coords, size, 0.0f, texture, 0.9f));
    nvg::Fill();
    nvg::ResetScissor();
    nvg::ClosePath();
}
...
void DrawBigMedals(CControlFrame@ BigMedalsParent)
{
    ...
    if (maxMedalIndex < 0) return;

    CControlLabel@ newMedal = cast<CControlLabel>(BigMedalsParent.Parent.Childs[0]);
    bool showNewMedal = data.locations.GotCheck(loadedMap.seriesIndex,loadedMap.mapIndex, leadType);
    if (showNewMedal)
    {
        newMedal.Show();
    }
    ...
    for (int i = 4; i >= 0; i--)
    {
        ...
        if (data.locations.GotCheck(loadedMap.seriesIndex, loadedMap.mapIndex,check))
        {
            if (maxMedalIndex < 0)
            {
                ...
                shifts[arrayIndex]  = Math::Abs(maxMedalIndex);
            }
            else
            {
                indexes[arrayIndex] = maxMedalIndex;
                checks[arrayIndex]  = check;
                shifts[arrayIndex]  = 0;
            }
...
bool DrawBigMedal (CControlQuad@ Medal, CheckTypes type, int shift = 0)
{
    ...
    ItemTypes textureIndex = loadedMap.itemTypes[int(type)];
    nvg::Texture@ texture = GetNthTex(textureIndex);
    ...
    nvg::Rotate(theta);
    nvg::Scale(mscale);
    nvg::FillPaint(nvg::TexturePattern(size * -0.5, size, 0.0f, texture, 1.0f));
    nvg::Fill();
    nvg::ResetTransform();
    nvg::ClosePath();
    return false;
...
void DrawBowTie (CControlQuad@ BowTie, int shifts)
{
    ...
    ItemTypes textureIndex = loadedMap.itemTypes[data.locations.GetNthCheck(loadedMap.seriesIndex, loadedMap.mapIndex, 0)];
    nvg::Texture@ texture = GetNthBottomTex(textureIndex);
    ...
    nvg::Translate(coords + size * 0.5);
    nvg::Scale(mscale);
    nvg::FillPaint(nvg::TexturePattern(size * -0.5, size, 0.0f, texture, 1));
    nvg::Fill();
    nvg::ResetTransform();
    nvg::ClosePath();
}
